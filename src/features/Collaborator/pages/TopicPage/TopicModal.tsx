    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent className="pt-6 space-y-4">
        ...
        >
          ...
          <MenuItem value="Part 4">Part 4</MenuItem>
          <MenuItem value="Part 5">Part 5</MenuItem>
          <MenuItem value="Part 6">Part 6</MenuItem>
          <MenuItem value="Part 7">Part 7</MenuItem>
        </TextField>
              ...
              >
                <IconComp
                  className={
                    formData.iconName === iconItem.name
                      ? "text-white"
                      : mapBgToIconColor(iconItem.bgColor)
                  }
                  sx={{ fontSize: 24 }}
                />
              </motion.button>
        ...
        <div className="grid grid-cols-4 gap-2">
          {gradientOptions.map((gradient, index) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() =>
                setFormData((prev) => ({ ...prev, gradient: gradient.value }))
              }
              className={`h-12 rounded-xl bg-gradient-to-br ${gradient.value} ${
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-xl">
          ...
          <div
            className={`h-24 bg-gradient-to-br ${formData.gradient} rounded-xl relative`}
          >
            <div className="absolute bottom-3 left-3">
              <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-lg">
                {iconFormData && (
                  <iconFormData.icon
                    className={mapBgToIconColor(formData.bgColor)}
                    sx={{ fontSize: 24 }}
                  />
                )}
              </div>
          ...
          <div className="mt-3">
            <h4 className="font-bold text-gray-900">
              {formData.title || "Tên chủ đề"}
            </h4>
            <p className="text-sm text-gray-600 mt-1">
              {formData.description || "Mô tả chủ đề"}
            </p>
          </div>
        </div>
      </DialogContent>
