...
export default function GroupsSection({ groups }: { groups: Group[] }) {
  ...
  const partNumbers = Array.from(new Set(groups.map((g) => Number(g.part)))).sort((a, b) => a - b);
  ...
  return (
    <Paper sx={{ p: 3, boxShadow: 1, borderRadius: 2 }}>
      ...
      {partNumbers.map((partNumber, idxPart) => {
        ...
        return (
          <Accordion key={partNumber} defaultExpanded={idxPart === 0} sx={{ mb: 1.5 }}>
            <AccordionSummary expandIcon={<ExpandMore />}>
              <Typography fontWeight="bold" color="primary">
                Part {partNumber} — {partGroups.length} nhóm, {partGroups.reduce((sum, g) => sum + (g.questions?.length || 0), 0)} câu
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              {partGroups.map((group, i) => (
                <GroupItem key={(group as any)._id || group.id || i} group={group} index={i} />
              ))}
            </AccordionDetails>
          </Accordion>
        );
